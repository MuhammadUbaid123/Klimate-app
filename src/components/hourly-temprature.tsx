...
const HourlyTemprature = ({ data }: HourlyTempratureProps) => {
    let chartData = data.list.slice(0, 8).map((item) => ({
                ...
                <div className="h-[200px] w-full">
                    <ResponsiveContainer width={"100%"} height={"100%"}>
                        <LineChart data={chartData}>
                            ...
                            {/* Both charts line here */}
                            <Line type="monotone" dataKey="temp" stroke="#2563eb" strokeWidth={2} dot={true} />
                            <Line type="monotone" dataKey="feels_like" stroke="#64748b" strokeWidth={2} dot={true} strokeDasharray={"5 5"} />
                        </LineChart>
                    </ResponsiveContainer>
                </div>
            </CardContent>
